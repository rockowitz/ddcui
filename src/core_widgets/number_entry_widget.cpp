...
{
   bool debug = false;
   TRACECF_STARTING(debug, "widget = %s, _curState = %d", QS2S(this->objectName()),_widgetState);
   ...
   TRACECF_DONE(debug, "_curState = %d", _widgetState);
}
      
#ifdef OLD
void
NumberEntryWidget::onTextEdited(const QString &text) {
   TRACECF_STARTING(true, "text=%s", text.toLatin1().data());
   bool ok = false;
   if (text.length() > 0) {
...
NumberEntryWidget::onTextEdited(const QString &text) {
   bool debug = false;
   TRACECF_STARTING(debug, "text=%s", text.toLatin1().data());
   ...
   //    _curval = newval;
   TRACECF_DONE(debug, "setting _widgetState to %d", _widgetState);
}
